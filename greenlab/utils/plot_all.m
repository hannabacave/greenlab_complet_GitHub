...
[nb_exp_days, nb_exp_leaves] = size(experimental_data.leaves_area);
...
for i=1:nb_leaves
    ...
    figure('Visible', 'off');
    if i <= nb_exp_leaves
        scatter(1:nb_exp_days,experimental_data.leaves_area(:,i), 's','DisplayName',"données") 
        hold on;
    end
    if i <= nb_simu_leaves
...
scatter(1:nb_exp_days,sum(experimental_data.leaves_area,2,'omitnan'), 's','DisplayName',"données SumLA") 
...
for i=2:nb_simu_leaves
    plot(1:nb_simu_days, organs.leaves.demand(:,i), '-','DisplayName',"Leaf"+(i-1));
    hold on
end
hold off
legend('show','Location', 'northwest')
